...
export const generateAffiliateLink = async ({ product_detail_url }: { product_detail_url: string }) => {
    ...
    if (!result?.promotion_links?.promotion_link?.length) {
        const message = result?.promotion_links?.promotion_link?.[0]?.message || "Affiliate link not available for this product";
